...
module.exports.run = async (client, message, args) => {
    ...
    if (!reason) return message.channel.send("You must give a reason for the bug!").then(msg => {
        message.delete()
    ...
    message.guild.channels.cache.forEach((channel) => {
        ...
        if (channel.name == userName.toLowerCase() + "-" + userDiscriminator) {
            ...
            message.channel.send("You already have a bug that is open!").then(msg => {
                message.delete()
                setTimeout(() => msg.delete(), 10000);
            });
    ...
    message.guild.channels.create(userName.toLowerCase() + "-" + userDiscriminator, { type: "text" }).then((createChan) => {
        ...
        createChan.setParent(categoryID).then((settedParent) => {
            ...
            settedParent.permissionOverwrites.edit(message.guild.roles.cache.find(x => x.id === `${process.env.DEVID}`), {
                VIEW_CHANNEL: true,
                CREATE_INSTANT_INVITE: false,
                READ_MESSAGE_HISTORY: true,
                SEND_MESSAGES: true,
            ...
            message.channel.send('✅ Your Bug has been created.').then(msg => {
                message.delete()
                setTimeout(() => msg.delete(), 10000);
            });
            ...
            message.channel.send('❌ There is an error when creating your Bug. **Contact an ADMIN**')
        })
...
module.exports.help = {
    name: "bug",
    category: "general",
    discription: "With this command you can create a bug."
}
